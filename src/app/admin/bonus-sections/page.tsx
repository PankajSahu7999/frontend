...
export default function AdminBonusSectionsPage() {
  ...
  const fetchSections = async () => {
    ...
    try {
      ...
      if (typeFilter !== 'all') {
        endpoint += `&section_type=${typeFilter}`;
      }
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
        {[
          ...
          { key: 'bonus', label: '🎁 Casino Bonuses' },
          { key: 'casino', label: '🏆 Casinos Directory' },
          { key: 'game', label: '🎲 Casino Games' },
          { key: 'slot', label: '🔥 Slots Hub' },
          { key: 'betting', label: '📺 Sports Betting' },
        ].map((tab) => (
          <button
            key={tab.key}
                    ...
                    <div className="flex items-center gap-2.5 flex-wrap mb-1">
                      ...
                      <span
                        className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border capitalize ${
                          section.section_type === 'casino'
                            ? 'bg-blue-50 text-blue-700 border-blue-200'
                            : section.section_type === 'game'
                            ? 'bg-purple-50 text-purple-700 border-purple-200'
                            : section.section_type === 'slot'
                            ? 'bg-amber-50 text-amber-700 border-amber-200'
                            : section.section_type === 'betting'
                            ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                            : 'bg-indigo-50 text-indigo-700 border-indigo-200'
                        }`}
                      >
                        {section.section_type || 'bonus'} hub
                      </span>
                      {section.badge_text && (
