...
export default function NewBonusSectionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/admin/bonus-sections'), {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setFormError(err.error || err.message || `Failed to save bonus section (Server HTTP ${res.status}).`);
      }
    } catch (err: any) {
      console.error(err);
    ...
    <div className="max-w-5xl mx-auto pb-20">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Create New Bonus Section</h1>
            <p className="text-sm text-slate-500 mt-1">
              Add a promotional category and configure related casino bonus offers.
            </p>
          </div>
          <span className="px-3 py-1 bg-amber-50 text-amber-800 border border-amber-200 rounded-full text-xs font-semibold">
            Bonus Section Builder
          </span>
        </div>
