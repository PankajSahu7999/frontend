    ...
    const fetchCategory = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setFeedback({ type: 'error', message: 'Failed to load category details.' });
        }
      } catch (err) {
        console.error(err);
        setFeedback({ type: 'error', message: 'Network error while fetching category.' });
      } finally {
        setIsFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setContentSections(updated.content_sections || payload.content_sections);
        setFeedback({ type: 'success', message: 'Category & content sections updated successfully!' });
        setTimeout(() => {
          router.push('/admin/categories');
        }, 1200);
      } else {
        const errorData = await res.json().catch(() => ({}));
        setFeedback({ type: 'error', message: errorData.error || 'Failed to save category. Please check your inputs.' });
      }
    } catch (err) {
      console.error(err);
      setFeedback({ type: 'error', message: 'Server connection error occurred while saving.' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Configure category name, slug, and bottom guide sections</p>
          </div>
        </div>
