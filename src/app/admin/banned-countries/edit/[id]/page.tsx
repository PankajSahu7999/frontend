...
export default function EditBannedCountryPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/banned-countries/${params.id}`);
        if (res.ok) {
    ...
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
          <h1 className="text-2xl font-bold text-slate-800">Edit Banned Country</h1>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <Input label="Country Name" value={formData.country_name} onChange={(e) => setFormData({...formData, country_name: e.target.value})} required />
          <Input label="Country Code" value={formData.country_code} onChange={(e) => setFormData({...formData, country_code: e.target.value})} required />
